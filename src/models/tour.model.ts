...
const tourSchema = new Schema(
  {
    ...
    ratingsAverage: {
      ...
      set: (val: number) => Math.round(val * 10) / 10, // rounding up decimals
    },
    ...
    guides: [{ type: Types.ObjectId, ref: 'User' }], // Child referencing
  },
  {
...
//   const tour = this as ITour;
//   return tour.duration / 7;
// });

// DOCUMENT MIDDLEWARE =>
// pre() - This runs before save() and create(), it doesn't run on insert().
...
// post() - This runs after save() and create(), it doesn't run on insert()
...
//   tour.start = Date.now(); // milliseconds
//   next();
// });
...
export default model<ITourDocument, ITourModel>('Tour', tourSchema);
